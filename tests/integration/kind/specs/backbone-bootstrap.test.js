...
import { startSitePortForward, waitForHostnamesEntry } from "../../helpers/site-api-client.js";
...
describe("backbone site bootstrap", () => {
    beforeAll(async () => {
        ...
        if (!secretExists(TEST_MANAGE_AP_TLS_SECRET)) {
            kubectl(["rollout", "restart", "deployment", SITE_CONTROLLER_DEPLOYMENT], {
                namespace: SITE_NAMESPACE,
            });
            await waitForRunningPod(SITE_CONTROLLER_LABEL, SITE_NAMESPACE);
        }

        await waitForPodLogMarkers(SITE_CONTROLLER_LABEL, SC_STARTUP_LOG_MARKERS, SITE_NAMESPACE);
    }, 600_000);
    ...
    it("Postgres has backbone site seeded and bootstrapped", () => {
        ...
        expect(state).toMatch(/^ready-boot|^deployed$|^colo-automatic$|^ready-automatic$/);
    });
