...
describe("api-user", () => {
    beforeEach(() => {
        ...
        mockClient.query.mockImplementation(async (sql, params) => {
            ...
            if (
                sql.includes(
                    "JOIN Backbones ON ApplicationNetworks.Backbone = Backbones.Id WHERE ApplicationNetworks.Id"
                )
            ) {
                return {
                    rowCount: 1,
    ...
    it("GET /vans returns 401 without authentication", async () => {
        ...
        await request(app).get("/api/v1alpha1/vans").expect(401);
    });
    ...
    it("GET /vans returns 403 without can-list-vans role", async () => {
        ...
        await request(app).get("/api/v1alpha1/vans").set("x-test-auth", "1").expect(403);
    });
});
