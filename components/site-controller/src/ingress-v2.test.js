...
import { META_ANNOTATION_STATE_ID } from "@skupperx/modules/common";
...
describe("ingress bundles", () => {
    ...
    it("returns empty bundles before any access points are tracked", async () => {
        const { GetAccessPointKind, GetIngressBundle, GetIngressBundleV2, GetInitialState } =
...
describe("access point watcher", () => {
    ...
    it("records router access endpoints and syncs local state", async () => {
        ...
        expect(UpdateLocalState).toHaveBeenCalledWith("accessstatus-ap-1", expect.any(String), {
            host: "router.example.com",
            port: 9090,
        });
    });
