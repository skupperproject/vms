    ...
    if (action != "DELETE") {
        ...
        try {
            ...
            if (result.rowCount == 1) {
                ...
                await client.query(
                    "UPDATE ManagementControllers SET Lifecycle = 'skx_cr_created' WHERE Id = $1",
                    [row.id]
                );
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            if (backbone.lifecycle == "new") {
                ...
                notify.add("CertificateRequests", cert.rows[0].id);
                await client.query(
                    "UPDATE Backbones SET Lifecycle = 'skx_cr_created' WHERE Id = $1",
                    [row.id]
                );
...
async function onAccessPointsChange(action, id) {
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            notify.add("CertificateRequests", cert.rows[0].id);
            await client.query(
                "UPDATE BackboneAccessPoints SET Lifecycle = 'skx_cr_created' WHERE Id = $1",
                [row.id]
            );
            notify.update("BackboneAccessPoints", row.id);
        }
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            if (van.lifecycle == "new") {
                ...
                notify.add("CertificateRequests", cert.rows[0].id);
                await client.query(
                    "UPDATE ApplicationNetworks SET Lifecycle = 'skx_cr_created', VanId = $1 WHERE Id = $2",
                    [van_id, van.id]
                );
...
async function onInteriorSitesChange(action, id) {
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            notify.add("CertificateRequests", cert.rows[0].id);
            await client.query(
                "UPDATE InteriorSites SET Lifecycle = 'skx_cr_created' WHERE Id = $1",
                [row.id]
            );
            notify.update("InteriorSites", row.id);
        }
...
const onInvitationsChange = async function (action, id) {
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            notify.add("CertificateRequests", cert.rows[0].id);
            await client.query(
                "UPDATE MemberInvitations SET Lifecycle = 'skx_cr_created' WHERE Id = $1",
                [row.id]
            );
            notify.update("MemberInvitations", row.id);
        }
...
async function onMemberSitesChange(action, id) {
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            notify.add("CertificateRequests", cert.rows[0].id);
            await client.query(
                "UPDATE MemberSites SET Lifecycle = 'skx_cr_created' WHERE Id = $1",
                [row.id]
            );
            notify.update("MemberSites", row.id);
        }
...
async function onNetworkCredentialsChange(action, id) {
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            notify.add("CertificateRequests", cert.rows[0].id);
            await client.query(
                "UPDATE NetworkCredentials SET Lifecycle = 'skx_cr_created' WHERE Id = $1",
                [row.id]
            );
            notify.update("NetworkCredentials", row.id);
        }
...
async function processCertificateRequests(nonrecurring) {
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            let cert_obj = certificateObject(
                name,
                row.durationhours,
                is_ca,
                issuer_name,
...
async function secretAdded(dblink, secret) {
    ...
    try {
        ...
        if (result.rowCount == 1) {
            ...
            if (is_ca) {
                var issuer_obj = issuerObject(
                    secret.metadata.name,
                    secret.metadata.annotations["skupper.io/skx-dblink"]
                );
