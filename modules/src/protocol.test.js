...
describe("protocol message builders", () => {
    ...
    it("AssertClaimResponseSuccess wraps claim acceptance payload", () => {
        expect(
            AssertClaimResponseSuccess("site-id", [{ id: "link-1" }], { kind: "Secret" })
        ).toEqual({
            statusCode: 200,
            statusDescription: "OK",
            siteId: "site-id",
...
describe("DispatchMessage", () => {
    ...
    it("rejects unsupported protocol versions and op codes", async () => {
        await expect(
            DispatchMessage({ version: 99, op: "HB" }, vi.fn(), vi.fn(), vi.fn())
        ).rejects.toThrow("Unsupported protocol version 99");

        await expect(
            DispatchMessage({ version: 1, op: "UNKNOWN" }, vi.fn(), vi.fn(), vi.fn())
