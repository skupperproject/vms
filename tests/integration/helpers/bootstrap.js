...
function siteControllerDeployed(namespace = SITE_NAMESPACE) {
  const { status } = kubectl(
    ["get", "deployment", SITE_CONTROLLER_DEPLOYMENT, "-o", "name"],
 ...
 * @returns {boolean}
 */
export function needsBackboneBootstrap(
  siteId = TEST_SITE_ID,
...
function interiorSiteDeploymentState(siteId) {
  ...
  return psql(
    `SELECT deploymentstate FROM interiorsites WHERE id = '${escaped}';`,
  ).trim()
}
...
/**
 * Wait until a label selector has at least one Running pod.
...
export async function waitForRunningPod(
  ...
  throw new Error(
    `Timed out waiting for Running pod (${labelSelector}) in ${namespace}. ${detail}`,
  )
}
...
function describePodFailure(labelSelector, namespace) {
  ...
  const init = pod.status?.initContainerStatuses?.[0]
  const parts = [`pod=${pod.metadata.name}`, `phase=${pod.status?.phase}`]
  if (init?.state?.waiting?.reason) {
    parts.push(`init=${init.state.waiting.reason}`)
  }
  if (init?.state?.terminated?.reason) {
    parts.push(
      `init=${init.state.terminated.reason} exit=${init.state.terminated.exitCode}`,
    )
  }
  try {
    const logs = kubectl(
...
export async function waitForPodLogMarkers(
  ...
  while (Date.now() < deadline) {
    ...
    if (failurePattern.test(logs)) {
      throw new Error(
        `Pod startup failed (${labelSelector}): matched ${failurePattern}`,
      )
    }
  ...
  throw new Error(
    `Timed out waiting for log markers on ${labelSelector}: ${markers.join(", ")}`,
  )
}
 ...
 * @param {number} [timeoutMs]
 */
export async function waitForAccessPointRow(
  apId,
  expected = {},
  timeoutMs = 300_000,
) {
  ...
  while (Date.now() < deadline) {
    ...
    const hostnameOk =
      expected.hostname === undefined || last.hostname === expected.hostname
